...
Users.log_in = (email, password, result) => {
  sql.query(
    `SELECT id,first_name,last_name,mobile,email,password,gender,CONCAT('` +
      nodeSiteUrl +
      `','/file/profile_image/',profile_picture) as profile_picture,country_id,state_id,city_id,device_type,device_token FROM users WHERE email=? AND password=?`,
    [email, password],
    (err, res) => {
      // console.log(res); return false
      result(null, res[0]);
...
Users.getuserinfo = (user_id, result) => {
  sql.query(
    `SELECT u.id,u.first_name,u.last_name,u.mobile,u.email,u.password,u.gender,CONCAT('` +
      nodeSiteUrl +
      `','/file/profile_image/',profile_picture) as profile_picture,u.country_id,c.name as country_name,u.state_id,s.name as state_name,u.city_id,ci.name as city_name,u.device_type,u.device_token FROM users as u LEFT JOIN countries as c ON u.country_id=c.id LEFT JOIN states as s ON u.state_id=s.id LEFT JOIN cities as ci  ON u.city_id=ci.id WHERE u.id=?`,
    [user_id],
    (err, res) => {
      //   console.log(res); return false
      result(null, res[0]);
...
Users.editprofile = (
  ...
  sql.query(
    `UPDATE users SET first_name=?,last_name=?,email=?,mobile=?,profile_picture=?  WHERE id=?`,
    [first_name, last_name, email, mobile, profile_picture, user_id],
    (err, res) => {
...
Users.updatepasswordbymail = (email, new_password, result) => {
  sql.query(
...
Users.checkadmin = (user_id, result) => {
  sql.query(`SELECT * FROM admin WHERE user_id=${user_id}`, (err, res) => {
    // console.log(res); return false
    result(null, res[0]);
    return;
  });
